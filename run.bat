...
if errorlevel 1 (
    echo [错误] Python未安装或未加入 PATH，请先安装/配置Python喵...
    echo.
    echo 请在命令行手动运行：python --version  检查是否可用喵...
    echo.
    echo 按任意键退出...
    pause >nul
    exit /b 1
...
if errorlevel 1 (
    ...
    echo 3. 磁盘空间不足喵...
    echo.
    echo 尝试安装 virtualenv 模块喵...
    python -m pip install virtualenv
    if errorlevel 1 (
        echo [错误] 安装 virtualenv 失败喵...
        echo.
        echo 按任意键退出...
        pause >nul
        exit /b 1
...
if not exist %VENV_DIR%\Scripts\activate.bat (
    echo [警告] 虚拟环境激活脚本不存在喵...
    echo.
    echo 将直接使用系统 Python 继续喵...
    goto :skip_venv
)
...
if not exist requirements.txt (
    ...
    if !SUCCESS! EQU 0 (
        ...
        echo       1. 网络连接问题喵...
        echo       2. 手动安装：pip install -r requirements.txt 喵...
...
if %PROGRAM_EXIT_CODE% NEQ 0 (
    echo [错误] 程序异常退出，错误代码: %PROGRAM_EXIT_CODE% 喵...
    echo.
    echo 可能原因:
    echo 1. Python 模块缺失喵...
    echo 2. 程序内部错误喵...
