...
class ReminderService:
    def __init__(self, message_handler: MessageHandler, mem_service: MemoryService):
        ...
        # self.web_socket = message_handler.web_socket
    ...
    def _remind_text_generate(self, remind_content: str, sender_name: str):
        core_mem = self.mem_service.get_core_memory(avatar_name=self.message_handler.current_avatar,
                                                    user_id=sender_name)
        context = self.mem_service.get_recent_context(avatar_name=self.message_handler.current_avatar,
                                                      user_id=sender_name)
    ...
    def add_reminder(self, chat_id: str, target_time: datetime, content: str, sender_name: str,
        ...
        try:
            ...
            if reminder_type == "voice":
                ...
                if audio_file_path is None:
                    ...
                    logger.info(
                        f"提醒任务已添加。提醒时间: {target_time}, 内容: {content}，用户：{sender_name}，类型：{reminder_type}")
